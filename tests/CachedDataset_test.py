...
def cachedDataset_test(sentences, concept, model_id, device):
    ...
    for sentence in data:
        for token in sentence:
            ...
            str_prompt = [
                tokenizer.decode(p) if isinstance(p, t.Tensor) else p for p in prompt
            ]
            token_prompt = [
                (
            ...
            attention_mask = (
                (t.tensor(token_prompt) != tokenizer.eos_token_id).long().to(device)
            )
    ...
    logs = training.train(
        dataloader, n_top_tracked_tokens=1, specified_tokens=[concept_id]
    )
    logit_set = []
    ...
    for i, tensor1 in tqdm(enumerate(logit_set)):
        for j, tensor2 in enumerate(comparison_logit_set):
            distance = (
                t.mean((tensor1.to(device) - tensor2.to(device)) ** 2)
                .detach()
                .cpu()
                .numpy()
            )
...
if __name__ == "__main__":
    sentences = ["the dog sat in the fog", "the cat sat on the mat"]
    concept = "animal"
    model_id = "distilgpt2"
