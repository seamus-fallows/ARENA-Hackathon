    ...
    def train(
        ...
        for epoch in tqdm(range(self.config.epochs)):
            for sentences, targets in dataloader:
                ...
                if epoch == 199:
                            embeddings = self.create_modified_embeddings(tokens, magic_token_pos)
                            final_token_logits = self.model(inputs_embeds=embeddings).logits[:,-1,:]
                            max_predicted_tokens = t.argmax(final_token_logits, dim=-1)
                            print(max_predicted_tokens.shape)
                            print(max_predicted_tokens)
                            print(f"max token: {self.tokenizer.decode(max_predicted_tokens)}, prob: {t.max(F.softmax(final_token_logits, dim=-1), dim=-1)}")
...
config = Config()
config.loss_coeffs = {"label": 1.0, "kl": 0.0, "entropy": 0.0}
config.lr = 0.0001
config.batch_size = 1
config.epochs = 200
