...
if experiment_new:
    prompt = dict(
        ...
        system_prompt="Your task is to assess if a given word from some text represents a specified concept in the context of the text. This could be a concept related to the meaning of the word, or its structural usage in the text. Provide a rating based on this assessment:\nIf the word represents the concept, respond with 'Rating: 1'.\nIf the word does not represent the concept, respond with 'Rating: 0'.\nThink carefully about if the concept applies to the word in the context of the text. Be confident.",
        # fmt: off
    ...
    data_path = "../data/text_data/repeated_words"
...
if experiment_new:
    ...
    solution = [
        tokenizer.encode("repeat")[-1],
        tokenizer.encode("repetition")[-1],
        tokenizer.encode("duplicate")[-1],
        tokenizer.encode("copy")[-1],
        tokenizer.encode("recurrence")[-1]
    ]
# %%
