...
if experiment_new:
    ...
    system_prompt="""Your task is to assess if a given word from some text is an example of or represents a particular concept. You will be given the information in the following format:
Text: <text>
Word: <word>
Concept: <concept>
You should ask yourself "is <word> an example of <concept>?". If the word could be considered an example of, or in some way represents the concept, reply with “Answer: Yes”. If the word is not an example of the concept, reply with “Answer: No”. Consider the semantic and syntactic aspects of the word, as well as its context and usage in the sentence.
...
if experiment_new:
    ...
    config.loss_coeffs = {"label": 1.0, "kl": 0.05, "entropy": 0.25}
    config.lr = 0.5
...
# %%
print(trainings_logs.top_tokens.values())

for token_id in trainings_logs.top_tokens.keys():
    
# %%
top_ids  = sorted(
            trainings_logs.top_tokens.keys(),
            key=lambda x: trainings_logs.top_tokens[x]["max_prob"],
            reverse=True,
        )
# %%
for id in top_ids:
