...
if experiment_new:
    prompt = dict(
        ...
        system_prompt="Your task is to assess if a given word belongs to a class of words. Provide a rating based on this assessment:\nIf the word is in the class, respond with 'Rating: 1'.\nIf not, respond with 'Rating: 0'.\n Be confident.",
        # fmt: off
        user_prompt=lambda sentence, concept, token: ["Is the word \"",token,"\" an example of the a \"" ,concept,"\"?"],
        # fmt: on
        ai_answer="Rating: ",
        yes_answer="1",
        no_answer="0",
    ...
    data_path = "../data/text_data/palindrome_words"
    datagenerator = generate_data.TextDataGenerator(
        file_path=data_path, tokenizer=tokenizer
    )
    datagenerator.print_only_words()
    datagenerator.print_sentences_with_words()
    datagenerator.print_sentences_without_words()
...
if experiment_new:
    ...
    solution = [tokenizer.encode("Palindrome")[-1], tokenizer.encode(" palindrome")[-1]]
